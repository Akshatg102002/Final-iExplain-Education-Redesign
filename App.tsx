        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {STUDENT_SERVICES_DATA.map((service, idx) => (
            <a
              ...
              className="group bg-white dark:bg-slate-800 p-8 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col justify-between"
            >
              {/* Icon */}
              <div>
                <div className="w-16 h-16 bg-blue-50 dark:bg-slate-700 rounded-2xl flex items-center justify-center text-brand-blue dark:text-white text-2xl mb-6 group-hover:bg-brand-blue group-hover:text-white transition-colors duration-300">
              ...
              {/* Read More */}
              <div className="mt-6 flex items-center text-brand-gold font-bold text-xs uppercase tracking-widest group-hover:translate-x-2 transition-transform duration-300">
                Read More
  ...
  if (!data) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-8 bg-white dark:bg-slate-900">
      <div className="w-20 h-20 bg-gray-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-6 text-gray-400">
        <i className="fa-solid fa-file-circle-question text-3xl"></i>
      </div>
      <h2 className="text-2xl font-black text-brand-blue dark:text-white mb-2">Exam Info Unavailable</h2>
      <p className="text-gray-500 font-medium">We are updating this information.</p>
    </div>
  );

  // Construct content if missing, ensuring sections are only added if data exists
  let contentHtml = data.content || '';

  if (!contentHtml) {
    const sections = [];
    if (data.overview) sections.push(`<h3>About the Exam</h3><p>${data.overview}</p>`);
    if (data.eligibility && data.eligibility.length > 0) {
      sections.push(`<h3>Eligibility Criteria</h3><ul>${data.eligibility.map((e: string) => `<li>${e}</li>`).join('')}</ul>`);
    }
    if (data.syllabus && data.syllabus.length > 0) {
      sections.push(`<h3>Syllabus</h3><ul>${data.syllabus.map((s: string) => `<li>${s}</li>`).join('')}</ul>`);
    }
    if (data.prepTips && data.prepTips.length > 0) {
      sections.push(`<h3>Preparation Tips</h3><ul>${data.prepTips.map((t: string) => `<li>${t}</li>`).join('')}</ul>`);
    }
    contentHtml = sections.join('');
  }

  // If still empty (sparse data), add a placeholder message
  if (!contentHtml.trim()) {
    contentHtml = `<p>Detailed information for ${data.title} is currently being updated. Please contact our counselors for the latest brochure and guidance.</p>`;
  }

  const adaptedData = {
    title: data.title,
    tagline: data.tagline || 'Entrance Exam Guide',
...
const AboutPage = () => <AboutSection compact={false} />;
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (route.view) {
      ...
      case 'exams': return <EntranceExamDetailPage data={EXAMS_DETAILED[route.subPath || 'neet-ug']} />;
