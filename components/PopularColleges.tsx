...
const PopularColleges: React.FC = () => {
  ...
  // ... (rest of the component state and useEffect)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-12">
          <div className="flex items-center justify-center space-x-8 overflow-x-auto no-scrollbar pb-6 px-4">
            {Object.entries(COUNTRY_ICONS).map(([name, code]) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center shrink-0 group transition-all ${
                    selectedCountry === name ? 'scale-110' : 'opacity-60 grayscale hover:opacity-100 hover:grayscale-0'
                  }`}
                >
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-3 transition-colors overflow-hidden ${
                    selectedCountry === name ? 'bg-brand-gold text-white shadow-xl shadow-brand-gold/20' : 'bg-gray-100 dark:bg-slate-800 text-brand-blue dark:text-white'
                  }`}>
                    {FlagComponent ? <FlagComponent className="w-8 h-8" /> : <span className="text-xs">{code}</span>}
                  </div>
                  <span className={`text-[10px] font-black uppercase tracking-widest ${
                    selectedCountry === name ? 'text-brand-gold' : 'text-gray-500'
                  }`}>
                    {name}
                  </span>
                </button>
